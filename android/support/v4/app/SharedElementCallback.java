...
public abstract class SharedElementCallback {
    ...
    static {
        MAX_IMAGE_SIZE = AccessibilityEventCompat.TYPE_TOUCH_INTERACTION_START;
    }
    ...
    public Parcelable onCaptureSharedElementSnapshot(View sharedElement, Matrix viewToGlobalMatrix, RectF screenBounds) {
        ...
        if (bitmapWidth > 0 && bitmapHeight > 0) {
            float scale = Math.min(TextTrackStyle.DEFAULT_FONT_SCALE, ((float) MAX_IMAGE_SIZE) / ((float) (bitmapWidth * bitmapHeight)));
    ...
    private static Bitmap createDrawableBitmap(Drawable drawable) {
        ...
        float scale = Math.min(TextTrackStyle.DEFAULT_FONT_SCALE, ((float) MAX_IMAGE_SIZE) / ((float) (width * height)));
        if ((drawable instanceof BitmapDrawable) && scale == TextTrackStyle.DEFAULT_FONT_SCALE) {
