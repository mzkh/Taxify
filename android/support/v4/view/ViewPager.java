...
public class ViewPager extends ViewGroup {
    ...
    static class C00541 implements Comparator<ItemInfo> {
        C00541() {
        }
    ...
    static class C00552 implements Interpolator {
        C00552() {
        }
    ...
    class C00563 implements Runnable {
        C00563() {
        }
    ...
    public static class SavedState extends BaseSavedState {
        ...
        static class C03291 implements ParcelableCompatCreatorCallbacks<SavedState> {
            C03291() {
            }
        ...
        static {
            CREATOR = ParcelableCompat.newCreator(new C03291());
        }
    ...
    class MyAccessibilityDelegate extends AccessibilityDelegateCompat {
        ...
        public void onInitializeAccessibilityEvent(View host, AccessibilityEvent event) {
            ...
            if (event.getEventType() == Connections.MAX_RELIABLE_MESSAGE_LEN && ViewPager.this.mAdapter != null) {
    ...
    static {
        ...
        LAYOUT_ATTRS = iArr;
        COMPARATOR = new C00541();
        sInterpolator = new C00552();
    ...
    public ViewPager(Context context) {
        ...
        this.mEndScrollRunnable = new C00563();
    ...
    void smoothScrollTo(int x, int y, int velocity) {
        ...
        float distance = ((float) halfWidth) + (((float) halfWidth) * distanceInfluenceForSnapDuration(Math.min(TextTrackStyle.DEFAULT_FONT_SCALE, (TextTrackStyle.DEFAULT_FONT_SCALE * ((float) Math.abs(dx))) / ((float) width))));
        velocity = Math.abs(velocity);
        if (velocity > 0) {
            duration = Math.round(1000.0f * Math.abs(distance / ((float) velocity))) * 4;
        } else {
            duration = (int) ((TextTrackStyle.DEFAULT_FONT_SCALE + (((float) Math.abs(dx)) / (((float) this.mPageMargin) + (((float) width) * this.mAdapter.getPageWidth(this.mCurItem))))) * 100.0f);
        }
    ...
    private void calculatePageOffsets(ItemInfo curItem, int curIndex, ItemInfo oldCurInfo) {
        ...
        this.mLastOffset = curItem.position == N + INVALID_POINTER ? (curItem.offset + curItem.widthFactor) - TextTrackStyle.DEFAULT_FONT_SCALE : AutoScrollHelper.NO_MAX;
        ...
        while (i < itemCount) {
            ...
            if (ii.position == N + INVALID_POINTER) {
                this.mLastOffset = (ii.widthFactor + offset) - TextTrackStyle.DEFAULT_FONT_SCALE;
            }
            ii.offset = offset;
            offset += ii.widthFactor + marginOffset;
            i += SCROLL_STATE_DRAGGING;
    ...
    public boolean dispatchPopulateAccessibilityEvent(AccessibilityEvent event) {
        if (event.getEventType() == Connections.MAX_RELIABLE_MESSAGE_LEN) {
