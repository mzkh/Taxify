...
class ActionBarActivityDelegateBase extends ActionBarActivityDelegate implements Callback {
    ...
    class C00891 implements Runnable {
        ...
        public void run() {
            ...
            if ((ActionBarActivityDelegateBase.this.mInvalidatePanelMenuFeatures & FileUploadPreferences.BATTERY_USAGE_UNRESTRICTED) != 0) {
    ...
    private static final class PanelFeatureState {
        ...
        void setStyle(Context context) {
            ...
            widgetTheme.resolveAttribute(C0093R.attr.actionBarPopupTheme, outValue, true);
            ...
            TypedArray a = context2.obtainStyledAttributes(C0093R.styleable.Theme);
            this.background = a.getResourceId(C0093R.styleable.Theme_panelBackground, 0);
            this.windowAnimations = a.getResourceId(C0093R.styleable.Theme_android_windowAnimationStyle, 0);
            a.recycle();
        }
        ...
        MenuView getListMenuView(MenuPresenter.Callback cb) {
            ...
            if (this.listMenuPresenter == null) {
                this.listMenuPresenter = new ListMenuPresenter(this.listPresenterContext, C0093R.layout.abc_list_menu_item_layout);
    ...
    class C03362 implements OnApplyWindowInsetsListener {
        C03362() {
        }
    ...
    class C03373 implements OnFitSystemWindowsListener {
        C03373() {
        }
    ...
    ActionBarActivityDelegateBase(ActionBarActivity activity) {
        super(activity);
        this.mInvalidatePanelMenuRunnable = new C00891();
    }
    ...
    final void ensureSubDecor() {
        if (!this.mSubDecorInstalled) {
            if (this.mHasActionBar) {
                ...
                TypedValue outValue = new TypedValue();
                this.mActivity.getTheme().resolveAttribute(C0093R.attr.actionBarTheme, outValue, true);
                if (outValue.resourceId != 0) {
                    themedContext = new ContextThemeWrapper(this.mActivity, outValue.resourceId);
                } else {
                    themedContext = this.mActivity;
                }
                this.mSubDecor = (ViewGroup) LayoutInflater.from(themedContext).inflate(C0093R.layout.abc_screen_toolbar, null);
                this.mDecorContentParent = (DecorContentParent) this.mSubDecor.findViewById(C0093R.id.decor_content_parent);
                ...
                if (this.mOverlayActionMode) {
                    this.mSubDecor = (ViewGroup) LayoutInflater.from(this.mActivity).inflate(C0093R.layout.abc_screen_simple_overlay_action_mode, null);
                } else {
                    this.mSubDecor = (ViewGroup) LayoutInflater.from(this.mActivity).inflate(C0093R.layout.abc_screen_simple, null);
                }
                if (VERSION.SDK_INT >= 21) {
                    ViewCompat.setOnApplyWindowInsetsListener(this.mSubDecor, new C03362());
                } else {
                    ((FitWindowsViewGroup) this.mSubDecor).setOnFitSystemWindowsListener(new C03373());
                }
            }
            ...
            ViewGroup abcContent = (ViewGroup) this.mSubDecor.findViewById(C0093R.id.action_bar_activity_content);
    ...
    private void applyFixedSizeWindow() {
        TypedValue tvw;
        TypedValue tvh;
        TypedArray a = this.mActivity.obtainStyledAttributes(C0093R.styleable.Theme);
        ...
        if (a.hasValue(C0093R.styleable.Theme_windowFixedWidthMajor)) {
            if (null == null) {
                mFixedWidthMajor = new TypedValue();
            }
            a.getValue(C0093R.styleable.Theme_windowFixedWidthMajor, mFixedWidthMajor);
        }
        if (a.hasValue(C0093R.styleable.Theme_windowFixedWidthMinor)) {
            if (null == null) {
                mFixedWidthMinor = new TypedValue();
            }
            a.getValue(C0093R.styleable.Theme_windowFixedWidthMinor, mFixedWidthMinor);
        }
        if (a.hasValue(C0093R.styleable.Theme_windowFixedHeightMajor)) {
            if (null == null) {
                mFixedHeightMajor = new TypedValue();
            }
            a.getValue(C0093R.styleable.Theme_windowFixedHeightMajor, mFixedHeightMajor);
        }
        if (a.hasValue(C0093R.styleable.Theme_windowFixedHeightMinor)) {
            if (null == null) {
                mFixedHeightMinor = new TypedValue();
            }
            a.getValue(C0093R.styleable.Theme_windowFixedHeightMinor, mFixedHeightMinor);
        }
    ...
    ActionMode startSupportActionModeFromWindow(ActionMode.Callback callback) {
        ...
        if (this.mActionModeView == null) {
            if (this.mIsFloating) {
                ...
                this.mActionModePopup = new PopupWindow(context, null, C0093R.attr.actionModePopupWindowStyle);
                this.mActionModePopup.setContentView(this.mActionModeView);
                this.mActionModePopup.setWidth(-1);
                TypedValue heightValue = new TypedValue();
                this.mActivity.getTheme().resolveAttribute(C0093R.attr.actionBarSize, heightValue, true);
                ...
                this.mShowActionModePopup = new C00904();
            } else {
                ViewStubCompat stub = (ViewStubCompat) this.mActivity.findViewById(C0093R.id.action_mode_bar_stub);
                if (stub != null) {
    ...
    int getHomeAsUpIndicatorAttrId() {
        return C0093R.attr.homeAsUpIndicator;
    }
    ...
    View createView(String name, @NonNull Context context, @NonNull AttributeSet attrs) {
        if (VERSION.SDK_INT < 21) {
            Object obj = -1;
        ...
        if (!st.isOpen && !isDestroyed()) {
            ...
            if (cb == null || cb.onMenuOpened(st.featureId, st.menu)) {
                ...
                if (wm != null && preparePanel(st, event)) {
                    ...
                    st.isHandled = false;
                    WindowManager.LayoutParams lp2 = new WindowManager.LayoutParams(width, -2, st.f0x, st.f1y, Place.TYPE_ADMINISTRATIVE_AREA_LEVEL_2, 8519680, -3);
    ...
    private boolean initializePanelMenu(PanelFeatureState st) {
        ...
        if ((st.featureId == 0 || st.featureId == 8) && this.mDecorContentParent != null) {
            ...
            baseTheme.resolveAttribute(C0093R.attr.actionBarTheme, outValue, true);
            Theme widgetTheme = null;
            if (outValue.resourceId != 0) {
                widgetTheme = context.getResources().newTheme();
                widgetTheme.setTo(baseTheme);
                widgetTheme.applyStyle(outValue.resourceId, true);
                widgetTheme.resolveAttribute(C0093R.attr.actionBarWidgetTheme, outValue, true);
            } else {
                baseTheme.resolveAttribute(C0093R.attr.actionBarWidgetTheme, outValue, true);
            }
            if (outValue.resourceId != 0) {
                if (widgetTheme == null) {
                    widgetTheme = context.getResources().newTheme();
            ...
            if (this.mActionModeView.isShown()) {
                ...
                if (mlp.topMargin != newMargin) {
                    ...
                    if (this.mStatusGuard == null) {
                        ...
                        this.mStatusGuard.setBackgroundColor(this.mActivity.getResources().getColor(C0093R.color.abc_input_method_navigation_guard));
