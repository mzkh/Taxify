...
public class ActionBarContextView extends AbsActionBarView implements ViewPropertyAnimatorListener {
    ...
    public ActionBarContextView(Context context, AttributeSet attrs) {
        this(context, attrs, C0093R.attr.actionModeStyle);
    }

    public ActionBarContextView(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        TintTypedArray a = TintTypedArray.obtainStyledAttributes(context, attrs, C0093R.styleable.ActionMode, defStyle, ANIMATE_IDLE);
        setBackgroundDrawable(a.getDrawable(C0093R.styleable.ActionMode_background));
        this.mTitleStyleRes = a.getResourceId(C0093R.styleable.ActionMode_titleTextStyle, ANIMATE_IDLE);
        this.mSubtitleStyleRes = a.getResourceId(C0093R.styleable.ActionMode_subtitleTextStyle, ANIMATE_IDLE);
        this.mContentHeight = a.getLayoutDimension(C0093R.styleable.ActionMode_height, (int) ANIMATE_IDLE);
        this.mSplitBackground = a.getDrawable(C0093R.styleable.ActionMode_backgroundSplit);
        this.mCloseItemLayout = a.getResourceId(C0093R.styleable.ActionMode_closeItemLayout, C0093R.layout.abc_action_mode_close_item_material);
        a.recycle();
    }
    ...
    private void initTitle() {
        ...
        int i2 = 8;
        if (this.mTitleLayout == null) {
            LayoutInflater.from(getContext()).inflate(C0093R.layout.abc_action_bar_title_item, this);
            this.mTitleLayout = (LinearLayout) getChildAt(getChildCount() - 1);
            this.mTitleView = (TextView) this.mTitleLayout.findViewById(C0093R.id.action_bar_title);
            this.mSubtitleView = (TextView) this.mTitleLayout.findViewById(C0093R.id.action_bar_subtitle);
    ...
    public void initForMode(ActionMode mode) {
        ...
        this.mClose.findViewById(C0093R.id.action_mode_close_button).setOnClickListener(new C00971(mode));
        ...
        if (MeasureSpec.getMode(widthMeasureSpec) != 1073741824) {
            ...
            if (this.mTitleLayout != null && this.mCustomView == null) {
                if (this.mTitleOptional) {
                    this.mTitleLayout.measure(MeasureSpec.makeMeasureSpec(ANIMATE_IDLE, ANIMATE_IDLE), childSpecHeight);
                    int titleWidth = this.mTitleLayout.getMeasuredWidth();
                    boolean titleFits = titleWidth <= availableWidth;
                    if (titleFits) {
                        availableWidth -= titleWidth;
                    }
                    this.mTitleLayout.setVisibility(titleFits ? ANIMATE_IDLE : 8);
                } else {
        ...
        if (this.mMenuView != null) {
            ...
            if (count > 0) {
                ...
                while (i >= 0) {
                    ...
                    ViewPropertyAnimatorCompat a = ViewCompat.animate(child).scaleY(TextTrackStyle.DEFAULT_FONT_SCALE);
                    a.setDuration(300);
                    set.play(a);
                    i--;
                    j += ANIMATE_IN;
    ...
    private ViewPropertyAnimatorCompatSet makeOutAnimation() {
        ...
        if (this.mMenuView != null && this.mMenuView.getChildCount() > 0) {
            for (int i = ANIMATE_IDLE; i < 0; i += ANIMATE_IN) {
                View child = this.mMenuView.getChildAt(i);
                ViewCompat.setScaleY(child, TextTrackStyle.DEFAULT_FONT_SCALE);
