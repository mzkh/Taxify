...
public class zza {
    ...
    public static int zzJ(Parcel parcel) {
        int zzI = zzI(parcel);
        int zza = zza(parcel, zzI);
        int dataPosition = parcel.dataPosition();
        if (zzaP(zzI) != 20293) {
            throw new zza("Expected object header. Got 0x" + Integer.toHexString(zzI), parcel);
        }
        zzI = dataPosition + zza;
        if (zzI >= dataPosition && zzI <= parcel.dataSize()) {
            return zzI;
        }
        throw new zza("Size read is invalid start=" + dataPosition + " end=" + zzI, parcel);
    }
